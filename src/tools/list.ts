...
export interface SourceSummary {
  ...
  /** Instrument type (treaty, guideline, regulation, etc.) */
...
function buildSourceMetadataSelect(db: Database): string {
  ...
  return [
    ...
    's.full_name',
    's.identifier',
    columns.has('authority') ? 's.authority' : 'NULL as authority',
    columns.has('instrument_type')
      ? 's.instrument_type'
      : 'NULL as instrument_type',
    columns.has('category') ? 's.category' : 'NULL as category',
    columns.has('jurisdiction') ? 's.jurisdiction' : 'NULL as jurisdiction',
    columns.has('source_url') ? 's.source_url' : 'NULL as source_url',
  ].join(',\n      ');
}
